  ...
  return new Promise((resolve, reject) => {
    ...
    const process = spawn('node', [scriptPath], {
      env: {
        ...
        DIRECTUS_URL: globalThis.process.env.DIRECTUS_URL || 'http://localhost:8055',
...
async function main() {
  const directusUrl = globalThis.process.env.DIRECTUS_URL || 'http://localhost:8055';
  ...
  try {
    ...
    try {
      await runScript('create-collections', '📋 Step 1/3: Creating Collections');
    } catch (error) {
