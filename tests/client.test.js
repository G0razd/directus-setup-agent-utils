...
testSuite.test('DirectusClient has required methods', () => {
  ...
  const requiredMethods = [
    'request', 'testConnection', 'getCollections', 'createCollection',
    'getItems', 'createItem', 'createItems', 'deleteItem', 'getFields'
  ];
...
testSuite.test('Token is stored securely (not exposed)', () => {
  const client = new DirectusClient('http://localhost:8055', 'secret-token-xyz');
...
testSuite.test('Client handles error responses', async () => {
  ...
  try {
    ...
    await client.testConnection().catch(e => {
      if (!e.message.includes('Unauthorized') && !e.message.includes('Connection failed')) {
